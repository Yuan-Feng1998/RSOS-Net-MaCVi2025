...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, feat_fuse0,feat_fuse1,feat_fuse2):
        inputs = torch.cat([feat_fuse0,feat_fuse1,feat_fuse2], dim=1)
        x = self.conv0(inputs)
        ###ch-attention
        ch_avgout=self.mlp(self.avg_pool(x))
        ch_maxout=self.mlp(self.max_pool(x))
        ch_add=ch_avgout+ch_maxout
        ch_sigmoid=self.sigmoid(ch_add)
        x_attn = x*ch_sigmoid
        #sp-attention
...
class RSOSHead(BaseDecodeHead):
    def __init__(self, fppm=dict(c1=512, c2=256),ffm=dict(
                     in_channels=768, out_channels=512, scale_factor=4), **kwargs):
